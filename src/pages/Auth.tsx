...
export default function AuthPage() {
  ...
  const redirectAfterLogin = async () => {
    ...
    if (roles.includes("program_director")) return navigate("/program-director", {
      replace: true
    });
    if (roles.includes("faculty")) return navigate("/faculty", {
      replace: true
    });
    if (roles.includes("resident")) return navigate("/resident", {
      replace: true
    });
    return navigate("/", {
      replace: true
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (mode === "login") await handleLogin();else await handleSignup();
  };
        ...
        <Card>
          ...
          <form onSubmit={onSubmit} className="space-y-4" aria-label={mode === "login" ? "Login form" : "Sign up form"}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedName} onValueChange={setSelectedName} disabled={!selectedRole}>
                      ...
                      <SelectContent className="z-50 max-h-64">
                        {(selectedRole === "resident" ? RESIDENTS : selectedRole === "faculty" ? FACULTY : []).map(n => <SelectItem key={n} value={n}>{n}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                </>}
            ...
            <div className="text-sm mt-4">
              {mode === "login" ? <button className="underline text-primary" onClick={() => setMode("signup")}>
                  Need an account? Sign up
                </button> : <button className="underline text-primary" onClick={() => setMode("login")}>
